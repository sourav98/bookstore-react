...
const PlaceOrder = (props) => {
    ...
    const   handleChange = (event) => {
      ...
      const handleSubmit = (event) => {
        ...
        OrderService.addOrder(orderDetails)
          .then((res) => {
            const orderId=res.data.orderDetailsId
             props.history.push(`/order/${orderId}/placed`);
          })
    ...
    return ( 
          <Hero title="Place Your Order"  className="container p-4"  description="Fill the details and order now">
...
<label>
 <input 
   name={bo.orderId}
   onChange={handleChange} value={bo.orderId} type="radio"  name="orderId"  class="form-check-input box"  id={bo.orderId}/>
   <div>
...
<i className="fas fa-shopping-cart"/> Place Order
      ...
      <div className="row">
          ...
          <div className="alert alert-danger" >
            <i className="fas fa-exclamation-triangle"></i> Sorry, currently book is out of stock.
          </div>
          </div>
          </div>
    )}
